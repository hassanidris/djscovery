...
import { MemoizedDjGigCard } from "./GigCard";
...
export function GigGrid({ gigs }: { gigs: GigItem[] }) {
  ...
  // Reset pagination when gigs array changes
  if (prevGigs !== gigs) {
    setPrevGigs(gigs);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {visible.map((gig) => (
          <MemoizedDjGigCard key={gig.slug} gig={gig} isDemo={!!gig.isDemo} />
        ))}
      </div>
