      ...
      <div className="flex flex-col gap-1 overflow-hidden rounded-xl border border-white/10">
        {PREFS.map((pref, i) => (
          <label
            key={pref.name}
            htmlFor={pref.name}
            className={`flex cursor-pointer items-center justify-between gap-4 px-5 py-4 transition-colors hover:bg-white/5 ${
...
function Toggle({
  ...
  return (
    <label className="relative inline-flex shrink-0 cursor-pointer items-center">
      ...
      />
      <div className="peer peer-checked:bg-h_red h-5 w-9 rounded-full bg-white/20 transition-colors after:absolute after:top-0.5 after:left-0.5 after:h-4 after:w-4 after:rounded-full after:bg-white after:shadow after:transition-transform peer-checked:after:translate-x-4" />
    </label>
  );
}
